...
The user asked for this AWS S3+CloudFront deploy GitHub Action — used nearly identically in `agentic-tekton`, `hekton-field-journal`, and now `coderturtle.io` — to be extracted into "the new site infra setup" as a reusable script/workflow rather than hand-copied a fourth time. The initial plan named `agentic-infra-lab` as the destination, but inspecting that lab plus `blog-factory-lab` showed the canonical reusable template already exists at `blog-factory-lab/infra/aws-static-site/github-actions/deploy-static-site.yml` — explicitly cited as the pattern origin by both `agentic-tekton`'s and this repo's own infra READMEs. `agentic-infra-lab` is the Infrastructure Gremlin's orchestration lab (preflight/plan/classify workflow docs) and consumes this pattern; it isn't where the template itself lives. Updated the actual template in `blog-factory-lab` instead of creating a second copy — see that repo's own `chore/harden-deploy-static-site-workflow` branch for the v1 hardening (push-trigger-live-by-default, vars-gated skip instead of a forgotten manual step, and a fix to cache assets by the real `_astro/` fingerprint directory instead of by file extension).
