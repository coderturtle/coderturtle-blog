...
## 2026-07-04 - No code fix for the reported "/projects/ points at the old coming-soon page"; add the old site's quote to the sitewide footer instead

User reported the live `/projects/` page was pointing at the old site's coming-soon page and separately said they liked that old page's footer quote ("Innovation is seeing what everybody has seen and thinking what nobody has thought") enough to want it folded into the Coderturtle brand.

Investigated the reported bug first: `curl` with a cache-busting query string against both `coderturtle.io` and `www.coderturtle.io`, plus a `dig` check confirming DNS resolves only to the new CloudFront distribution's IPs, all show the current, correct Astro-built `/projects/` page (`last-modified` matching the latest deploy, `x-cache: RefreshHit/Hit from cloudfront` with `cache-control: public,max-age=0,must-revalidate` so no stale edge cache is plausible). This matches the root cause already recorded in `docs/next-actions.md` for 2026-07-03: a prior "still seeing the old site" report traced to local browser cache, not a server-side fault. No code or infra change made for this part — nothing reproduced. Old projects page previously lived at `.old-site-backup-2020/projects/index.html` (S3 bucket for that distribution was already decommissioned per the 2026-07-03 next-action).

For the quote: added it to `src/layouts/BaseLayout.astro`'s sitewide footer (new `.site-footer-quote` line above the existing "Built by Coderturtle..." row), mirroring exactly where it sat on the old 2020 site (a footer tagline on every page) rather than the homepage's existing cranky one-liner quote-bubble rotation (`turtleQuotes` in `src/pages/index.astro`) — that array is deliberately dry/snarky per the voice guide, and this quote's earnest/aspirational register would clash with it. Verified via `npm run build` (clean) and by curling the Astro dev server's rendered output for both the compiled `.site-footer-quote` CSS and the literal quote text in the page body.

## 2026-07-18 - Built a real `/workshops/` page, closing half of the long-open `/enter/` stub-station next-action

`/enter/`'s workshops-turtle station has read "still being built" since the turtle-picker UI shipped — a real, previously-tracked gap (`docs/next-actions.md`), not new scope invented this session. User asked for a workshops listing on `coderturtle.io` linking to the real workshop subdomains, with mini marketing copy per workshop and tags for differentiation as more get added.

**Decision:** a new `workshops` content collection (`src/content.config.ts`), modeled on the existing `projects` collection's shape (title/tagline/summary/status/tags/order/featured) but with fields specific to a workshop rather than a build: `moduleProgress` (a real, honest completion state, not aspirational — e.g. "2 of 8 modules real," not "in progress") and `liveUrl` (the workshop's own subdomain, always external — these are separate repos/deploys, not part of this site). `status` reuses `shipped`/`building` only (not the full `projects` enum) since those are the only two states that actually apply to a workshop's content-completion, and reusing them means zero new CSS — the existing `.status-shipped`/`.status-building` pill styles just work.

`src/pages/workshops/index.astro` mirrors `projects/index.astro`'s card-grid pattern (`.note-card`/`.tag-list`), plus a client-side tag-filter button bar (vanilla JS, no framework, `aria-pressed` state) — the "easily searchable as we create more" requirement, kept lightweight rather than a full search box given only 4 workshops exist today. `/enter/`'s workshops-turtle now links to `/workshops/`; `Workshops` added to `BaseLayout.astro`'s sitewide nav (between Projects and Workbench).

**Before writing any marketing copy, checked each workshop's own README for its real, current completion state** rather than assuming all four are equally "done" — found two (`borrow-native`: 2/8 modules, `half-life`: 0/6 modules, all skeleton) are genuinely still in progress, not shipped. Status/`moduleProgress` reflect that honestly rather than oversold, matching this whole factory's own no-overclaiming discipline.

**A real, live bug found and fixed as a precondition, not scope creep:** `borrow-native.coderturtle.io` and `half-life.coderturtle.io` both 404'd on every path (confirmed via `curl`, not assumed) — DNS/GitHub Pages config was live, but each repo's own `site/astro.config.mjs`/`public/CNAME` cutover (the same fix `terminal-velocity`/`closed-book` already had) was never done. Fixed both in their own repos before linking to them here — shipping a Workshops page with two dead links would have been worse than not shipping it. Both now confirmed live and deployed end-to-end (first real `workflow_dispatch` run plus a confirmed push-triggered run for each).

**Consequences:** `npm run build` clean, generated HTML verified directly (all 4 cards present, correct `liveUrl`s, correct status pills) — but **no real browser check was done this session** (no browser-automation tool was available), unlike this project's own established practice of browser-verifying UI changes before calling them done. Flagged explicitly in `docs/next-actions.md` rather than silently skipped.

## 2026-07-18 (cont'd) - Fixed the info-toggle bug the user caught in a real browser check, then ran the summaries through the Editor Gremlin

User did the browser check this session couldn't do, and caught two real bugs: every workshop card's expandable summary rendered open by default, and the "i" toggle button did nothing. Both traced to the same root cause: `<p class="workshop-summary" ... hidden>` relied on the bare HTML `hidden` attribute, but `global.css`'s `.note-card p { display: block }` rule has higher CSS specificity (`.note-card p` = 0,1,1 vs. `[hidden]`'s default UA-stylesheet 0,1,0) and silently won, so `hidden` never actually hid anything — toggling it in JS was a real no-op, not a JS wiring bug. **Fixed by switching to an explicit class** (`.workshop-summary { display: none }` / `.workshop-summary.is-expanded { display: block }`, toggled via `classList.toggle`) instead of the attribute, which sidesteps the specificity fight entirely rather than trying to out-specificity the existing rule.

**Separately, user asked to run the summary copy through "our editorial gremlin."** Checked rather than assumed (same discipline as the earlier doc-cleanup-workflow investigation this session): the thing actually named "Editorial Gremlin" (`blog-factory-lab/agent-teams/editorial-gremlin/`) turned out to be a pipeline *dispatcher* — it decides which of Brief/Blogger/Promotion Gremlin owns a blog post's next stage, and explicitly does no drafting or editing itself. The real match for "professional technical editor/writer review" is the **Editor Gremlin role** inside Blogger Gremlin's own roster (`blog-factory-lab/agent-teams/blogger-gremlin/prompts/03-editor-gremlin.md`) — a critique-only contract (findings list with severity/location, never a rewrite) enforcing that team's `quality-bar.md`.

Applied that exact quality bar to all four workshops' `title`/`tagline`/`summary` fields: **one hard-fail finding** (an em dash in `terminal-velocity.md`'s summary — "any em dash is a blocker" per the bar), fixed by splitting into two sentences. Substance/Voice/Safety sections all passed on inspection: theses are stated in one sentence each, claims trace to real sources (half-life's Chroma citation, closed-book's non-affiliation disclaimer required by its own `docs/brand.md`), no AI-tell vocabulary or hype language found. Final verdict: pass, no findings remaining.
